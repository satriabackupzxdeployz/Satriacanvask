...
export async function POST(req: NextRequest) {
  try {
    ...
    const { username = "Player", rank = "imo", border = 0, avatar } = body;

    const validRanks = Object.keys(RANK_CONFIG);
    const safeRank = validRanks.includes(rank) ? rank : "imo";
    const safeName = String(username).trim().slice(0, 15) || "Player";
    const safeBorder = Number(border);
    ...
    const outPath = path.join(tmpDir, `sc_ml_${Date.now()}.png`);

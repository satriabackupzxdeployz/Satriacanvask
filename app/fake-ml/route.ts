  ...
  try {
    ...
    if (!username || !username.trim()) {
      return NextResponse.json(
        { error: "Parameter 'usr' wajib diisi", example: "/fake-ml?usr=PlayerName&rank=imo&border=1" },
        { status: 400 }
      );
    }
    ...
    const borderNum = border ? Number(border) : 0;
    if (border && (isNaN(borderNum) || borderNum < 1 || borderNum > 16)) {
      return NextResponse.json(
        { error: "Parameter 'border' harus angka antara 1–16", valid_range: "1-16" },
        { status: 400 }
      );
    }
    ...
    const lobbyImg = await loadImage(path.join(ASSETS, "Lobby.jpg"));
    ...
    const tmpPath = path.join(os.tmpdir(), `sc_ml_pub_${Date.now()}.png`);
    ...
    return new NextResponse(imgBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="ml-${safeName}-${safeRank}.png"`,
